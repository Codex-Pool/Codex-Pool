export default {
  accounts: {
    ...
    details: {
      ...
      officialDescription:
        "这里展示 OpenAI 官方模型信息，只读。下方可以编辑手工价格覆盖。",
      limitsTitle: "限额详情",
    ...
    messages: {
      ...
      toggleUnsupported:
        "当前后端版本不支持账号启用/禁用接口，请升级 control-plane。",
  ...
  billing: {
    antigravity: {
      ...
      noActivityDescription:
        "当前租户 {{tenant}} 还没有充值或用量扣费记录，所以趋势图和账本会保持为空。先准备账号池或发起请求后，这里会开始出现真实账单轨迹。",
      noTenant: "暂无可用租户",
      noTenantDescription:
        "请先创建或同步一个租户，然后这里才能展示真实的 credits 汇总与账本流水。",
      rechargeEvents: "充值事件",
      requestCountHint: "当前时间窗口内纳入成本汇总的请求总数。",
      scopeTitle: "范围",
      scopePanelDescription:
        "在租户与统计粒度之间切换，快速对齐你正在查看的费用窗口。",
    ...
    ledger: {
      ...
      details: {
        ...
        tokenSettlement:
          "代币结算：输入{{input}} + 缓存{{cached}} + 输出{{output}}",
  ...
  config: {
    ...
    themeLab: {
      title: "Theme Lab",
      description:
        "让默认视觉更贴近 HeroUI Pro 原生，并在当前浏览器里做轻量微调。",
      localOnly: "仅当前浏览器生效",
      currentMode: "当前模式：{{value}}",
      resolvedMode: "实际生效：{{value}}",
      modeTitle: "主题模式",
      modeDescription:
        "切换浅色、深色或跟随系统。这里会直接驱动全局 HeroUI 主题，而不是页面局部样式。",
      ...
      densityTitle: "页面密度",
      densityDescription:
        "调整全局间距与壳层留白，观察 HeroUI Pro 工作台的呼吸感。",
      footer:
        "这些设置会即时作用到当前管理台，并保存在本地浏览器。运行时配置的保存与重置不会影响这里。",
      reset: "恢复 HeroUI Pro 默认",
      previewTitle: "实时预览",
      previewDescription:
        "用一组标准卡片、表单和操作按钮，快速检查当前基线是否贴近官方风格。",
      previewMetric: "预览指标",
  ...
  dashboard: {
    ...
    modelDistribution: {
      a11y: {
        model: "模型",
        summary:
          "模型分布包含 {{count}} 行，按 {{mode}} 排序。领先模型：{{top}}。后附无障碍数据表。",
    ...
    healthSignals: {
      title: "健康信号",
      description:
        "按原因分类区分健康容量、限额冷却、瞬时故障、致命问题和人工操作。",
      healthy: "健康",
    ...
    tokenTrend: {
      a11y: {
        summary:
          "Token 趋势覆盖 {{count}} 个时间点，范围从 {{start}} 到 {{end}}。后附无障碍数据表。",
        summaryEmpty: "当前筛选条件下暂无 Token 趋势数据。",
        tableLabel: "Token 使用趋势数据表",
        timestamp: "时间",
      },
      description:
        "对比输入、缓存、输出和推理 Token 的时间变化，可按组件开关定位消耗从哪里升高。",
      empty: "暂无 Token 趋势数据",
      title: "Token 使用趋势",
    },
    title: "服务总览",
  ...
  importJobs: {
    ...
    antigravity: {
      ...
      intakeHint:
        "上传多份 .json/.jsonl 文件后，后端会把它们合并为同一个导入任务，并继续在左侧队列里追踪状态。",
      newImport: "新建导入",
      pause: "暂停",
      recentHint:
        "集中查看最近追踪的导入任务，并从同一队列恢复已暂停任务或重试失败项。",
      resume: "恢复",
      status: {
        paused: "已暂停",
      },
    ...
    dropzone: {
      acceptsNew:
        "支持单次上传多个 .json/.jsonl 文件，后端会自动汇总为同一个导入任务。",
      ...
      title: "点击上传或拖拽文件到此处",
      accepts:
        "支持 <code>.csv</code> 或换行分隔的 <code>.txt</code> 格式。单文件最大 50,000 行。",
      browse: "浏览文件",
    ...
    credentialMode: {
      title: "导入凭证模式",
      description:
        "选择这一批账号按可轮转的 refresh_token 导入，还是按一次性的 access_token 导入。OAuth 登录导入保持不变。",
      refreshToken: "导入 RT",
    ...
    subtitleModern:
      "先批量加入文件，预检查后再一键导入，并实时查看每个任务的进度和错误明细。",
    template: {
      downloadJsonl: "下载模板",
      title: "导入模板",
      ...
      titleNew: "导入模板",
      descNew:
        "下载 JSONL 模板，并根据当前选择的凭证模式填写 refresh_token 或 access_token。",
    },
    title: "批量导入任务",
    description:
      "上传凭据文件、跟踪准入结果，并在同一个工作台审计每一条导入记录。",
    validation: {
  ...
  oauthImport: {
    ...
    wizard: {
      label: "导入向导",
      description:
        "按步骤完成 OAuth 登录、回调兜底和结果确认，让账号顺畅进入账号池。",
      progress: "流程进度",
      current: "当前步骤",
      completed: "已完成",
    ...
    authorize: {
      ...
      helperTitle: "处理提示",
      helperDescription:
        "如果浏览器拦截了弹窗，或者自动回调迟迟没有到达，下一块里就可以直接提交手动回调 URL。",
    },
    form: {
      label: "账号标签（可选）",
    ...
    monitor: {
      ...
      activeLabel: "导入仍在进行中",
      activeHint:
        "可以留在此页继续观察，也可以稍后去账号池查看。左侧状态卡会持续自动刷新。",
    },
    actions: {
  ...
  logs: {
    ...
    messages: {
      ...
      modelsProbed:
        "模型探测（{{trigger}}）：通过 {{label}} 测试 {{tested}} 个模型（可用 {{available}}，不可用 {{unavailable}}）",
    ...
    request: {
      ...
      description:
        "范围：原始数据平面请求事件（状态/延迟/路径/租户/API 密钥/请求 ID）。",
      empty: "没有可用的请求日志数据",
      filters: {
    ...
    events: {
      title: "统一事件流",
      description:
        "在一个工作台里查看请求、账号池、巡检、导入、基础设施和管理动作事件。",
      ...
      insightsTitle: "事件聚焦",
      insightsDescription:
        "先看最常见的事件类型和原因码，再决定是追 request_id 还是直接打开详情。",
    ...
    time: {
      displayMode:
        "当前按本地时间（{{timezone}}）显示，悬浮提示和导出中保留 UTC 原值。",
  ...
  models: {
    ...
    dialog: {
      description:
        "在此对话框中编辑配置文件和定价。保存的定价将立即写回模型池列表。",
      officialDescription:
        "这里展示 OpenAI 官方模型信息，只读。下方可以编辑手工价格覆盖。",
    ...
    antigravity: {
      ...
      summaryTitle: "目录总览",
      summaryDescription:
        "从可用性、提供商和目录覆盖面上快速判断当前模型池状态。",
      catalogTitle: "探测与同步状态",
      catalogDescription:
        "这里集中展示 probe cache、新鲜度与官方目录同步状态。",
      maintenance: "目录维护",
      ...
      catalogAttentionSyncRequired:
        "官方目录还没有完成最新同步。请执行目录同步，再检查模型目录是否恢复完整。",
      catalogAttentionRetry:
        "最近一次目录同步或探测没有完成。当前页面保留了状态摘要，详细技术原因请到统一事件流继续排查。",
      catalogAttentionCacheStale:
        "探测缓存已经过期。建议重新执行一次可用性探测，避免按过期结果判断模型状态。",
      directoryTitle: "模型目录",
      directoryDescription:
        "按提供商和可用性筛选模型，再查看定价与官方元数据。",
    ...
    hints: {
      cannotDeleteMissingPricing:
        "当前模型没有本地定价数据，请先补充后再删除。",
      cannotDeleteNonLocalEntity:
        "当前模型不是本地实体模型，因此无法删除其实体。",
    },
    ...
    rules: {
      ...
      ruleSummary:
        "阈值 {{threshold}} · 输入 ×{{inputMultiplier}} · 缓存 ×{{cachedMultiplier}} · 输出 ×{{outputMultiplier}}",
      scope: "作用域",
      scopes: {
        request: "单次请求",
        session: "会话",
      ...
      sectionDescription:
        "为长上下文或特殊计费 band 配置按请求/会话生效的倍率规则。",
      sectionTitle: "分段计价规则",
      thresholdInputTokens: "输入 Token 阈值",
    },
    probeSourceUnknown: "未知账号",
    probeSummary:
      "探测缓存：{{stale}}，最近 {{checkedAt}}，缓存时长 {{ttlHours}} 小时，来源 {{source}}",
    syncHint: {
  ...
  accountPool: {
    ...
    recentSignal: {
      ...
      bucketTooltip:
        "{{time}} · {{count}} 条信号 · 主动 {{active}} / 被动 {{passive}}",
      legend: {
        success: "成功",
        mixed: "部分失败",
        error: "全部失败",
    ...
    detail: {
      ...
      test: {
        description:
          "对当前 runtime 账号发起一次真实的 Responses 会话请求，结果按用户请求记账，不计入主动巡检。",
  ...
  errors: {
    ...
    api: {
      ...
      refreshTokenReused:
        "Refresh token 已复用，请重新获取最新 refresh token。",
  ...
  proxies: {
    ...
    antigravity: {
      ...
      authNone: "无",
      emptyDescription:
        "添加第一个代理节点后，系统才能通过管理员维护的代理池转发流量。",
      latency: "延迟",
      scheme: "协议",
    },
    badges: {
    ...
    deleteDialog: {
      confirm: "删除代理",
      description:
        "确定要从全局出站代理池中删除 {{label}} 吗？现有请求会在下一次刷新后停止使用它。",
      title: "删除代理",
    },
    editor: {
      create: "创建代理",
      createTitle: "新增出站代理",
      description:
        "配置一个全局出站代理节点。编辑时如果留空代理 URL，则继续沿用当前密钥与凭证。",
      editTitle: "编辑出站代理",
      enabledHint:
        "禁用后的节点仍会保留在列表中，但不会参与选路，也不会自动测试。",
      errors: {
      ...
      proxyUrlHint:
        "支持 http://、https://、socks5://。必须包含主机和端口，用户名密码可直接写在 URL 中。",
    ...
    list: {
      description:
        "在这里增删改测加权代理节点。管理端会保存明文密钥，但对外只返回脱敏后的 URL。",
      title: "代理节点列表",
    },
    loading: "正在加载出站代理池…",
    meta: {
    ...
    settings: {
      description:
        "这些设置会作用于平台内所有外部 HTTP 与 WebSocket 出站请求。",
      enabled: "启用出站代理池",
      enabledHint:
        "关闭后，所有外部流量保持直连；开启后，流量会从下方的加权代理池中选择。",
      failMode: "失败策略",
      save: "保存设置",
      title: "全局代理池设置",
    },
    ...
    subtitle:
      "为所有上游流量配置统一的全局出站代理池。这一页已经不再是旧的数据面节点占位页。",
    title: "出站代理池",
  },
  system: {
    antigravity: {
      ...
      debugSignalsDescription:
        "把当前自动切号、鉴权校验、缓存与计费对账信号集中在一个视图里。",
      disabled: "已关闭",
      edition: {
        business: "企业版",
        personal: "个人版",
      ...
      runtimeConfigDescription:
        "这里聚合了当前实例真正生效的控制面、数据面与 OAuth 刷新配置。",
      runtimeCounts: "运行时计数",
    ...
    observability: {
      ...
      title: "故障切换与缓存观测",
      unavailableDesc:
        "请检查 control-plane 到 /internal/v1/debug/state 的访问与权限配置。",
  ...
  tenantApiKeys: {
    ...
    preview: {
      ...
      description:
        "当前分组：{{name}} · 输入 {{input}} · 缓存 {{cached}} · 输出 {{output}}",
      empty: "暂无可用分组。",
    ...
    auth: {
      ...
      error: {
        invalidCredentialsOrUnverified:
          "登录失败。请检查邮箱和密码；如果是首次登录，请先完成邮箱验证。",
  ...
  tenantBilling: {
    ...
    groupPricing: {
      ...
      description:
        "查看每个 API Key 当前使用的计价分组，并按单个 API Key 检查有效模型价格。",
  ...
  groupsPage: {
    ...
    antigravity: {
      ...
      listTitle: "分组目录",
      listDescription:
        "用统一账号分组策略组织 API Key、模型覆盖范围和计费倍率。",
      metrics: {
        total: "分组总数",
      ...
      serviceUnavailableDescription:
        "当前运行形态没有提供 API Key 分组存储，因此这一页暂时不可用。",
      updatedAt: "更新于 {{value}}",
    },
    filters: {
      statusLabel: "状态筛选",
    ...
    preview: {
      basePricingSummary:
        "{{provider}} · {{title}} · 基准价：输入 {{input}} · 缓存 {{cached}} · 输出 {{output}}",
      columns: {
        finalPrice: "最终价格",
        formulaPrice: "公式价格",
        mode: "模式",
  ...
  modelRoutingPage: {
    title: "模型调度",
    subtitle:
      "配置路由画像、模型感知回退链，以及用于模型调度规划的规划模型链。",
    actions: {
      refresh: "刷新",
      createProfile: "新建画像",
      createPolicy: "新建策略",
    ...
    settings: {
      ...
      plannerModelChainHint:
        "从模型池中选择规划兜底模型，并按从高优先级到低优先级保留顺序。",
    ...
    profiles: {
      title: "路由画像",
      description:
        "可复用的账号选择器，用来描述哪些 plan、模式和鉴权方式可以承接请求。",
      empty: "暂无路由画像。",
      summary:
        "计划 {{plans}} 项 · 模式 {{modes}} 项 · 鉴权 {{authProviders}} 项 · 强制包含 {{include}} 项 · 强制排除 {{exclude}} 项",
      anyMode: "全部账号模式",
    },
    policies: {
      title: "模型策略",
      ...
      empty: "暂无模型路由策略。",
      summary:
        "精确模型 {{exact}} 个 · 前缀 {{prefixes}} 个 · 回退画像 {{fallbacks}} 个",
    ...
    form: {
      ...
      exactModelsHint:
        "从模型池中选择精确模型 ID，配置时可同时看到可用状态和价格摘要。",
      modelPrefixes: "模型前缀",
      modelPrefixesPlaceholder: "gpt-5, o3",
      modelPrefixesHint:
        "这里保留自由输入，用于像 gpt-5 或 o3 这样的前缀匹配规则。",
    ...
    messages: {
      ...
      claudeCodeSettingsSaved: "Claude Code 路由设置已保存。",
      claudeCodeSettingsSaveFailed: "保存 Claude Code 路由设置失败。",
    ...
    errorLearning: {
      settings: {
        title: "上游错误学习",
        description:
          "审核首次发现的上游失败模板，在固化为确定性错误规则前先进行把关。",
        enabled: "启用上游错误学习",
      ...
      templates: {
        title: "模板审核队列",
        description:
          "查看临时模板和待审核模板，并对其进行批准、拒绝、编辑或 AI 重写。",
        empty: "暂无上游错误模板。",
        fingerprint: "错误指纹",
      ...
      builtinTemplates: {
        title: "内置模板",
        description:
          "查看算法默认文案和网关错误文案，并支持编辑、AI 重写或恢复系统默认值。",
        empty: "暂无内置模板。",
        kind: "模板类型",
        code: "模板代码",
        scope: "作用范围",
  ...
  tenantLogs: {
    ...
    time: {
      displayMode:
        "当前按本地时间（{{timezone}}）显示，悬浮提示中保留 UTC 原值。",
      tooltip: "本地时间：{{local}} | UTC：{{utc}}",
    },
    tabs: {
      audit: "审计日志",
  ...
  apiKeys: {
    ...
    createPanelDescription:
      "为当前单机工作区创建可调用 Data Plane 的访问密钥。创建后会返回一次明文 key，请立即保存。",
    create: "创建密钥",
    search: "搜索密钥名称或前缀…",
    loading: "加载凭据中…",
